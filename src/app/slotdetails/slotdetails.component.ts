...
Time: ${this.formatDateTime(slot.slotTime, 'hh:mm a')} - ${this.formatDateTime(slot.endTime, 'hh:mm a')}`;
 ...
 this.route.queryParams.subscribe(params => {
  ...
  this.mockTestInterviewId= (Number(mockTestInterviewId));
...
Time: ${this.formatDateTime(slot.slotTime, 'hh:mm a')} - ${this.formatDateTime(slot.endTime, 'hh:mm a')}`;
  ...
  private confirmBooking(slotId: number, userId: number): void {
    ...
    this.mockTestService.bookSlot(slotId, userId).subscribe(
      {next:response => {
        ...
        this.loadAvailableSlots(this.mockTestInterviewId);
      },
      error:(error) => {
  ...
  loadBookedSlots(userId: number): void {
    this.mockTestService.getBookedSlotsByUserId(userId).subscribe(
      {next:(slots: BookedMockTestInterview[]) => {
  ...
  getSlotTimeRange(slotName: string): string {
    ...
    const formattedStart = this.formatBookedDateTime(startDateTime, 'dd/MM/yyyy, hh:mm a');
    const formattedEnd = this.formatBookedDateTime(endDateTime, 'hh:mm a');
 ...
 formatDate(dateString: string): string {
    ...
    // Remove comma before time and make AM/PM lowercase
