...
// project
 ...
 searchText = '';
  ...
  ngOnInit() {
    ...
    { validators: [this.reviewMeetAfterStartValidator] });
 ...
 onStudentSelectionChange1(event: any, student: any) {
  ...
  futureDateValidator(control: any): { [key: string]: boolean } | null {
    ...
    return date.isAfter(moment()) ? null : { pastDate: true };
  }

  futureOrPresentValidator(control: any): { [key: string]: boolean } | null {
    const date = moment(control.value);
    return date.isSameOrAfter(moment(), 'day') ? null : { pastOrInvalidDate: true };
  }

  reviewMeetAfterStartValidator(group: FormGroup): { [key: string]: boolean } | null {
    const startDate = moment(group.get('startDate')?.value);
    const reviewMeetDate = moment(group.get('reviewMeetDate')?.value);
    const projectDeadline = moment(group.get('projectDeadline')?.value);
  
    if (!reviewMeetDate.isSameOrAfter(startDate)) {
      return { invalidReviewDate: true }; // Review Meet Date is earlier than Start Date
    }
  
    if (!reviewMeetDate.isAfter(projectDeadline)) {
      return { reviewBeforeDeadline: true }; // Review Meet Date is earlier than Project Deadline
    }
...
onSaveQuiz(): void {
  if (this.quizForm.valid && this.selectedStudents.length > 0) {
    ...
    const now = moment(); // Get the current date & time
    ...
    if (moment(startDate).isBefore(now)) {
      alert('Start date & time cannot be earlier than the current date & time.');
      return;
    }
    ...
    if (!courseId || !quizName || !teacherId) {
...
onSubmitliveclass(): void {
  ...
  // Convert the scheduled time to a moment object
  const scheduledTimeMoment = moment(this.scheduledTime);

  // ❌ Scheduled time must be in the future
  if (scheduledTimeMoment.isBefore(now)) {
    alert('Scheduled time cannot be earlier than the current date & time.');
    return;
  }
              ...
              if (item.types && item.types.length > 0) {
                item.types.forEach((typeEntry: PostedTypeEntry) => {
                  if (typeEntry.type === 'Assignment' && typeEntry.ids && typeEntry.ids.length > 0) {
                    ...
                    console.log('Quiz IDs:', typeEntry.ids);
                  }
                });
              }
            });
...
toggleVisibility(section: string): void {
  ...
  this.quizzesVisible = false;
 
  // Show the clicked section
  if (section === 'assignments') {
    this.assignmentsVisible = true;
  } else if (section === 'projects') {
    this.projectsVisible = true;
  } else if (section === 'quizzes') {
    this.quizzesVisible = true;
  }
}
}
