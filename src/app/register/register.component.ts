...
export class RegisterComponent implements OnInit {
  ...
  fullNameValidator(control: AbstractControl): ValidationErrors | null {
    ...
    const isValidLength = control.value.length >= 3 && control.value.length <= 30;
  ...
  register(): void {
      ...
      this.http.post(`${baseUrl}/user/register`, registrationData)
        .subscribe({
          ...
          error: (error: HttpErrorResponse) => {
            ...
            this.showPopup = true; // Hide popup if registration fails
            if (error.status === 409) {
    ...
    navigateToHrLogin(event: Event): void {
