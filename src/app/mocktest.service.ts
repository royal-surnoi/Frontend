...
export class MockTestService {
  ...
  getMockTests(): Observable<MockTestInterview[]> {
    return this.http.get<MockTestInterview[]>(`${this.apiBaseUrl}/getAllAvailableSelectedFields`).pipe(
      tap(response => console.log('Mock Tests Response:', response)),
      catchError(error => {
        console.error('Error fetching mock tests:', error);
        return throwError(() => new Error('Error fetching mock tests'));
      })
    );
  }
...
submitProjectByMock(studentId: number, projectId: number, mockId: number, file: File, userAnswer: string): Observable<SubmitAssignmentResponse> {
  ...
  return this.http.post<any>(`${this.apiBaseUrl}/submitProjectByMock`, formData)
    .pipe(
      catchError(this.handleError)
    );
}
...
getMocksByTeacherId(teacherId: number): Observable<any> {
  const url = `${this.apiBaseUrl}/getmockbyteacher/${teacherId}`;
  return this.http.get(url);
}
 
//////////////////////////////////////////////////////
getMockDetails(teacherId: number): Observable<any> {
  return this.http.get<any>(`${this.apiBaseUrl}/getMockDetails/${teacherId}`);
}

removeMock(mockId: number, teacherId: number): Observable<string> {
 return this.http.delete(`${this.apiBaseUrl}/DeleteMock/${mockId}/${teacherId}`, { responseType: 'text' });
}

getMockDetailsByMockId(mockId: number): Observable<any> {
  const url = `${this.apiBaseUrl}/getMockDetailsByMockId/${mockId}`;
  return this.http.get<any>(url);
}

editMock(mockId: number, teacherId: number, updatedMock: any): Observable<any> {
  return this.http.put(`${this.apiBaseUrl}/EditMock/${mockId}/${teacherId}`, updatedMock, { responseType: 'text' });
}
