...
export class LearningPageComponent implements OnInit {
  ...
  get skeletonArray(): number[] {
    return Array(this.isLoading ? 6 : 3).fill(0);
  }
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(async(params) => {
      this.searchTerm = await params['searchTerm']; // Ensure jobId is a number
      this.searchCourses();
    });
    this.userId = Number(localStorage.getItem('id'));
  ...
  fetchRecommendations(): void {
    ...
    if (cachedResponse) {
        ...
        alert('Home recommendations fetched successfully (from cache).');
        return;
    }
    ...
    this.aimlservicecourserec.getHomeRecommendations(this.userId).pipe(
      mergeMap((courses:any[]) => {
        ...
        return forkJoin(recommendations);
      }),
      shareReplay({ bufferSize: 1, refCount: false }),
      catchError(() => of([]))
    ).subscribe(
  ...
  getSpecialRecommendations(userId: number): Observable<string> {
    const url = `${environment.apiBaseUrlAI}/edu_recommendations`;
  ...
  sanitizedImage: SafeUrl | undefined;

  
  getimage(image:string){
    const base64Image = 'data:image/png;base64,';  // Your base64 string
    this.sanitizedImage = this.sanitizer.bypassSecurityTrustUrl(base64Image+image);
