...
export class ArticleService {
  ...
  private apiUrl = 'http://localhost:8080/api/articleposts';
 ...
 /api/comments/article/${articleId}`);
  ...
  addComment(articleId: number, comment: string): Observable<any> {
    return this.http.post<any>(`http://localhost:8080/api/comments/article/${articleId}`, { content: comment });
  }
  getTotalCommentsByArticlePostId(articlePostId: number): Observable<number> {
    return this.http.get<number>(`http://localhost:8080/api/count/articlepost/${articlePostId}`);
  }
}
