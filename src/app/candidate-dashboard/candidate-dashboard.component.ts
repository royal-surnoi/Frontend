...
loadAllData(): void {
  ...
  //   switch (status) {
  //     case 'completed': return '#452758';
  //     case 'in_progress': return '#452758';
  //     case 'not_started': return '#452758';
  //     default: return '#452758';
  //   }
  // }
  ...
  getStatusColor(status: string): string {
    switch (status) {
      case 'completed':
        return '#00FF00'; // Green for completed
      case 'in_progress':
        return '#FFFF00'; // Yellow for in progress
      case 'not_started':
        return '#FF0000'; // Red for not started
      default:
        return '#452758'; // Default color
    }
}
  ...
  loadProjects(): void {
    ...
    this.fusion2Service.getProjectsByCourse(this.courseId).subscribe(
      {next:(projects: any[]) => { // Use 'any' type for the raw response
  ...
  loadAssessmentProgress(): void {
    ...
    this.fusion2Service.getAssessmentProgress(+this.userId, this.courseId).subscribe(
      {next:(response: { progressPercentage?: number }) => {
  ...
  loadQuizzes2(lessonId: number): void {
    if (this.userId) {
      this.fusion2Service.getQuizProgressDetails(lessonId, this.userId).subscribe(
        {next:(details: QuizProgressDetails) => {
