...
updatePersonalDetails(personalDetailsId: number, personalDetails: any): Observable<any> {
  const url = `${this.apiBaseUrl}/personalDetails/updatePersonalDetails/${personalDetailsId}`;
  return this.http.put(url, personalDetails, { responseType: 'text' as 'json' }).pipe(
    catchError(error => {
      console.error('Error updating personal details:', error);
      return throwError(() => new Error('Error updating personal details.'));
    })
  );
}
