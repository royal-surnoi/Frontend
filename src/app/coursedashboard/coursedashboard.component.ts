...
export interface LessonModule18 {
  id: number;
  lessons?: Lesson[];
  lessonTitle:string;
...
updateCurrentLesson() {
  ...
  generateTranscript(): void {
    ...
    this.http.post(`http://localhost:8000/transcribe`,{}).subscribe(
      {next:(response) => {
...
clearChat() {
  ...
  console.log(request);
  this.http.post('http://localhost:8000/chat/new_chat', request).subscribe();
  this.chatHistory = [];
}
...
sendMessage() {
  ...
  this.http.post<any>('http://localhost:8000/chat/chat', request).subscribe({
    next: (response) => {
