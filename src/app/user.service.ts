...
export class UserService {
  ...
  // private apiUrl = 'http://localhost:8080/user/all';
  private apiUrl = 'http://localhost:8080/suggestFriendsRecommendations'
  ...
  private apiUrll = 'http://localhost:8080/api/users';
 
 private followRequestsUrl = 'http://localhost:8080/follow/zero-followers-following/followings'

 private apiBaseUrlAI = 'http://localhost:8000'
 ...
 private followedUsersSubject = new BehaviorSubject<number[]>([]);
  ...
  fetchUsers() {
    ...
    forkJoin({
      suggestedUsers: this.http.post<number[]>(`${this.apiBaseUrlAI}/suggest_friends`, { user_id: currentUserId }).pipe(
        tap(ids => console.log('Suggested user IDs received:', ids))
      ),
      ...
      tap(({ suggestedUsers, friends, followRequests }) => {
        console.log('Initial API responses:', { suggestedUsers, friends, followRequests });
      }),
      switchMap(({ suggestedUsers, friends, followRequests }) => {
        ...
        console.log('Filtered user IDs to fetch:', filteredUserIds);
        ...
        return forkJoin(
          filteredUserIds.map(id =>
            forkJoin({
              ...
              map(({ userDetails, followingCount }): User => ({
                ...userDetails,
                followingCount,
      ...
      tap(usersWithCounts => {
        console.log('Final processed users:', usersWithCounts);
  ...
  removeFollower(followerId: number, userId: number): Observable<any> {
    ...
    return this.http.delete('http://localhost:8080/follow/deleteByIds', { params }).pipe(
      tap(() => {
  ...
  updateFollowerCount(userId: number) {
    this.getFollowerCount(userId).subscribe(
     {next: result => {
      ...
      error:error => console.error('Error updating follower count:', error)
    });
  }
  updateFollowingCount(userId: number) {
    this.getFollowingCount(userId).subscribe(
     {next: result => {
      ...
      error:error => console.error('Error updating following count:', error)
    });
  }
  ...
  setUserBanner(userId: number, file:File ): Observable<any> {
  ...
  usersendOtp(email: string): Observable<string> {
    const params = new HttpParams().set('email', email);
    const baseUrl = `${this.apiBaseUrl}/user/generate-otp-change-password`; // Use this.apiBaseUrl
...
userresetPassword(email: string, otp: string, newPassword: string): Observable<string> {
  const params = new HttpParams()
    .set('email', email)
    .set('otp', otp)
    .set('newPassword', newPassword);
 
  const baseUrl = `${this.apiBaseUrl}/user/change-password-with-otp`;
...
forgotMentorUsername(email: string): Observable<string> {
  const params = new HttpParams().set('email', email);
  const baseUrl = `${this.apiBaseUrl}/user/mentor/forgot-username`; // Use this.apiBaseUrl
...
mentorSendOtp(email: string): Observable<string> {
  const params = new HttpParams().set('email', email);
  const baseUrl = `${this.apiBaseUrl}/user/mentor/generate-otp-for-forgot-password`;
...
mentorResetPassword(email: string, otp: string, newPassword: string): Observable<string> {
  const params = new HttpParams()
    .set('email', email)
    .set('otp', otp)
    .set('newPassword', newPassword);
 
  const baseUrl = `${this.apiBaseUrl}/user/mentor/change-password`;
