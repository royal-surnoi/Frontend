...
import { StudentdashboardService, CourseDocuments} from '../studentdashboard.service';
...
export class StudentDashboardComponent implements OnInit {
  ...
  courseDocuments: (CourseDocuments & { safeUrl?: SafeResourceUrl })[] = [];
  errorMessage: string = '';
  selectedCourseDocument: SafeResourceUrl | null = null;
...
isReportVisible: boolean = false;
  ...
  now: Date = new Date(); // Current time
...
getEnrolledCourses(id: any): Observable<any[]> {
 ...
 getProgressColor(progress: number): string {
  ...
  fetchCoursesWithDetails(userId: any) {
      ...
      mergeMap(courses => {
        const courseDetailsRequests = courses.map((course: any) => {
          if (course.course.courseType === 'online') {
            return this.http.get<any>(`http://localhost:8080/api/training-rooms/rooms/${course.course.id}`).pipe(
              map(trainingRoom => {
                console.log('🔹 Training Room Data:', trainingRoom); // ✅ Debug API response
                // alert("Pre transformcourse")
                const transformedCourse = {
                  ...course,
                  isOnline: true,
                  courseTitle: trainingRoom[0].course.courseTitle,
                  conferenceUrl: trainingRoom[0].conferenceUrl,
                  scheduledTime: trainingRoom[0].scheduledTime // ✅ Debug here
                };
  
                console.log('✅ Transformed Course:', transformedCourse); // ✅ Debug final object before return
                // alert("succufluy transfermed course")
                return transformedCourse;
              }),
              catchError(error => {
                console.error('❌ Error fetching training room: love you', error);
                alert("error")
                return of({ 
                  ...course, 
                  isOnline: true, 
            ...
            return of({ ...course, isOnline: false });
          }
        });
    ...
    ).subscribe(coursesWithDetails => {
      this.enrolledCourseList = coursesWithDetails;
  
      console.log('📌 Final Enrolled Courses:', this.enrolledCourseList); // ✅ Debug final list
  
      this.enrolledCourseList.forEach((course:any) => {
        console.log(`📅 Course: ${course.courseTitle}, Scheduled Time:`, course.scheduledTime); // ✅ Debug each course
      });
    });
  }
  ...
  // Function to check if Join button should be enabled
  isJoinButtonEnabled(scheduledTime: string): boolean {
    const now = new Date();
...
/////////////getting course document in resours

getCourseDocuments(): void {
  ...
  this.dashboardService.getUserEnrolledCourseDocuments(this.userId)
    .subscribe({
      next: (data) => {
        // Map each document to add a safe URL for the iframe
        this.courseDocuments = data.map(doc => ({
          ...doc,
          safeUrl: this.sanitizer.bypassSecurityTrustResourceUrl(
            `data:application/pdf;base64,${doc.courseDocument}`
          )
        }));
        console.log("Course Documents", this.courseDocuments);
      },
      error: (error) => {
        console.error('Error fetching course documents', error);
        this.errorMessage = 'No course documents found for this user.';
      }
    });
}
 ...
 * Accepts the SafeResourceUrl directly.
 */
openDocumentInNewWindow1(safeUrl: any): void {
  // Find the corresponding document in your courseDocuments array.
  // Option 1: If you already have the Base64 string stored in the doc, pass it along.
  // For example, if your safeUrl was created from doc.courseDocument, you'll need doc.courseDocument.
  
  // If safeUrl is not enough, you may need to modify your code to pass both.
  // For this example, I'll assume we can find the doc by safeUrl.
  const doc = this.courseDocuments.find(d => d.safeUrl === safeUrl);
  if (doc && doc.courseDocument) {
    const blobUrl = this.base64ToBlobUrl(doc.courseDocument);
...
/**
 * Triggers a download for the document.
 */
downloadFile1(courseId: number): void {
  const doc = this.courseDocuments.find(d => d.id === courseId);
  if (doc) {
    const link = document.createElement('a');
    link.href = `data:application/pdf;base64,${doc.courseDocument}`;
    link.download = `document-${courseId}.pdf`;
    link.click();
  } else {
    this.errorMessage = 'Document not found for this course.';
  }
}

private base64ToBlobUrl(base64Data: string, contentType: string = 'application/pdf'): string {
  const byteCharacters = atob(base64Data);
