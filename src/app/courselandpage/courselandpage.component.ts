...
items :any;
  ...
  getCourseDetails(id: number): void {
    this.fusion2service.getCourseDetails(id).subscribe({
      next: (course) => {
        this.course = course;
        console.log(course);
      },
...
isLocked(){
 ...
 lockedClose(){
  ...
  async ngOnInit(): Promise<void> {
   ...
   getData(courseId:any) {
    ...
    this.http.get(`${environment.apiBaseUrl}/courseTools/course/` + courseId).subscribe((res) => {
      ...
      this.imageUrl = (`data:image/png;base64,${this.tools[0].toolImage}`);
  ...
  console.log("levelKeys",this.levelKeys,this.learn)
    ...
    this.http.get(`${environment.apiBaseUrl}/course/enrollments/` + courseId).subscribe({
      next: (res: any) => {
...
showLessons2(module: any, moduleId: any) {
  if (this.selectedModule2 !== moduleId) {
    ...
    if (this.courseTerm === 'long') {
      this.courselandpageService.getLessonsByModule(moduleId).subscribe({
        next: (res) => {
          this.lessonList2 = res;
          console.log("Lesson List:", this.lessonList2); // Log the lessons
        },
        error: (error) => {
 ...
 returncourseDetails(courseId:any){
  ...
  getModuleByCourseId(courseId: any) {
    this.courselandpageService.getModulesByCourse(courseId).subscribe({
      next: (res) => {
