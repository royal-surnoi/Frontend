...
import { BehaviorSubject, Observable, Subject, Subscription, catchError, finalize, firstValueFrom, forkJoin, map, mergeMap, of, takeUntil } from 'rxjs';
...
import { User} from '../usersprofile.service';
...
interface CombinedPost extends Post {
  type: PostType; // Keep this for compatibility
  isVideo?: boolean;
  isImage?: boolean;
  isArticle?: boolean;
  normalizedDate?: Date;
...
this.activeTab = activeTab;
   ...
   ngOnInit(): void {
    ...
    this.route.params.pipe(
     ...
     this.currentUserId = String(this.userId);
  ...
  async updateSentRequestUsers(): Promise<void> {
   ...
   clickout(event: Event) {
    ...
    for (const openDropdown of Array.from(dropdowns)) {
       ...
       this.fusionService.getUserprofileById(userId).subscribe({
         next: (data) => {
           ...
           if (data.bannerImage != null) {
             const sanitizedUrl = this.sanitizer.bypassSecurityTrustUrl(`data:image/png;base64,${data.bannerImage}`);
   ...
   private async fallbackShareShort(short: any) {
    try {
      ...
      try {
        // Use execCommand as a fallback, but with a try/catch
        successful = document.execCommand('copy');
        if (!successful) throw new Error('Copy command failed');
      } catch (e) {
        console.error('Unable to copy to clipboard', e);
        alert('Failed to copy link. Please copy this URL manually: ' + short.s3Url);
      }
   ...
   private async fallbackShare(post: Post) {
    try {
      // Use the modern Clipboard API
      await navigator.clipboard.writeText(post.url);
      alert('Link copied to clipboard. You can now paste it to share.');
      
      // Only uncomment if copying to clipboard should count as a share
      // this.incrementShareCount(post);
    } catch (err) {
      // Fallback for browsers that don't support Clipboard API
      const textArea = document.createElement('textarea');
      ...
      // Position the textarea outside the viewport
      textArea.style.position = 'fixed';
      textArea.style.left = '-999999px';
      textArea.style.top = '-999999px';
      document.body.appendChild(textArea);
      ...
      try {
        ...
        console.error('Unable to copy to clipboard', e);
        alert('Failed to copy link. Please copy this URL manually: ' + post.url);
      }
   ...
   transformToPostFormat(item: any, type: 'video' | 'image' | 'article'): Post {
     ...
     let shortVideoLikes = type === 'video' ? item.shortVideoLikes || 0 : 0;
     let shortVideoShares = type === 'video' ? item.shortVideoShares || 0 : 0;
     let shortVideoViews = type === 'video' ? item.shortVideoViews || 0 : 0;
   ...
   saveBannerChanges() {
     if (this.tempBannerImage && this.selectedFile) {
       ...
       this.userService.setUserBanner(Number(this.userId), this.selectedFile).subscribe({
         next: (profile) => {
           console.log(profile)
           const sanitizedUrl = this.sanitizer.bypassSecurityTrustUrl(`data:image/png;base64,${profile.bannerImage}`);
            this.bannerImage = `url('${sanitizedUrl.toString().replace("SafeValue must use [property]=binding: ", "").replace(" (see https://g.co/ng/security", "")}')`; 
         },
         error: (error) => {
           console.log(error);
