...
constructor(private http: HttpClient){
  ...
  //getting course doc in resource
  getUserEnrolledCourseDocuments(id: any): Observable<CourseDocuments[]> {
    return this.http.get<CourseDocuments[]>(`${environment.apiBaseUrl}/users/${id}/course-documents`);
  }
  ...
  getCourseCompletionFraction(userId: any, courseId: any, courseTerm: any): Observable<any> {
    ...
    if (courseTerm === 'long') {
      ...
      return throwError(() => new Error('Invalid courseTerm value'));
    }
  }
  ...
  getUpcomingAssignment(id:any){
    return this.http.get(`${environment.apiBaseUrl}/upcoming?userId=${id}`)
  }
