...
savePost(saveEndpoint: string, userId: number, postId: number, postIdParam: string): Observable<any> {
    return this.http.post(
      ...
      catchError(error => {
        ...
        return throwError(() => new Error('An error occurred while saving the post.'));
      })
    );
  }
...
///////////////
  ...
  getUserprofileById(id: string): Observable<any> {
    return this.http.get(`${this.apiBaseUrl}/personalDetails/get/user/${id}`);
  }

  getUserEduactionById(id: string): Observable<any> {
    return this.http.get(`${this.apiBaseUrl}/education/user/${id}`);
  }

  getUserExperianceById(id: string): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiBaseUrl}/workExperience/user/${id}`);
  }

  uploadCourseDocuments(courseId: number, files: File[]): Observable<any> {
    const formData: FormData = new FormData();
    ...
    return this.http.post(`${this.apiBaseUrl}/${courseId}/documents`, formData);
  }

   
  deleteModule(moduleId: number): Observable<any> {
    return this.http.delete(`${this.apiBaseUrl}/deleteModule/${moduleId}`);
  }
